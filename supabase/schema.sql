-- OpenLeads CRM — schema de referência (copiar no SQL Editor do Supabase)
-- O arquivo oficial versionado está em: migrations/20260723214443_initial_schema.sql

create extension if not exists "pgcrypto";

create type public.user_role as enum ('user', 'admin');
create type public.lead_category as enum (
  'restaurante', 'pizzaria', 'hamburgueria', 'grafica', 'academia',
  'clinica', 'salao_beleza', 'oficina', 'loja', 'outros'
);
create type public.lead_status as enum ('novo', 'contatado', 'interessado', 'cliente', 'perdido');
